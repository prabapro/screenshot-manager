...
// .wrangler/tmp/pages-LbX9Gy/functionsWorker-0.13161661627240173.mjs
...
async function handleLogin(request, env22) {
  try {
    ...
    if (username !== AUTH.USERNAME || password !== env22.AUTH_PASSWORD) {
      return errorResponse(
...
async function handleListScreenshots(env22) {
  try {
    const bucket = env22.screenshots;
    if (!bucket) {
      return errorResponse(
    ...
    console.error("List screenshots error:", error32);
    return errorResponse(
      ERROR_MESSAGES.INTERNAL_ERROR,
      HTTP_STATUS.INTERNAL_SERVER_ERROR,
      error32.message
...
__name2(handleListScreenshots, "handleListScreenshots");
async function handleGetScreenshot(key, env22) {
  try {
    const bucket = env22.screenshots;
    if (!bucket) {
      return errorResponse(
    ...
    console.error("Get screenshot error:", error32);
    return errorResponse(
      ERROR_MESSAGES.INTERNAL_ERROR,
      HTTP_STATUS.INTERNAL_SERVER_ERROR,
      error32.message
...
__name2(handleGetScreenshot, "handleGetScreenshot");
async function handleDeleteScreenshot(key, env22) {
  try {
    const bucket = env22.screenshots;
    if (!bucket) {
      return errorResponse(
    ...
    console.error("Delete screenshot error:", error32);
    return errorResponse(
      ERROR_MESSAGES.INTERNAL_ERROR,
      HTTP_STATUS.INTERNAL_SERVER_ERROR,
      error32.message
...
__name2(handleDeleteScreenshot, "handleDeleteScreenshot");
async function authenticate(request, env22) {
  try {
    ...
    const payload = await verifyToken(token, env22.JWT_SECRET);
    if (!payload) {
      return errorResponse(
    ...
    console.error("Authentication error:", error32);
...
__name2(isErrorResponse, "isErrorResponse");
async function onRequest({ request, env: env22 }) {
  try {
    ...
    if (method === "POST" && pathname === API_ROUTES.AUTH_LOGIN) {
      return handleLogin(request, env22);
    }
    if (method === "POST" && pathname === API_ROUTES.AUTH_LOGOUT) {
      return handleLogout();
    }
    const authResult = await authenticate(request, env22);
    if (isErrorResponse(authResult)) {
      return authResult;
    }
    const user = authResult;
    if (method === "GET" && pathname === API_ROUTES.SCREENSHOTS) {
      return handleListScreenshots(env22);
    }
    if (method === "GET" && pathname.startsWith(`${API_ROUTES.SCREENSHOTS}/`)) {
      ...
      return handleGetScreenshot(decodeURIComponent(key), env22);
    }
    if (method === "DELETE" && pathname.startsWith(`${API_ROUTES.SCREENSHOTS}/`)) {
      ...
      return handleDeleteScreenshot(decodeURIComponent(key), env22);
    }
    return errorResponse(ERROR_MESSAGES.NOT_FOUND, HTTP_STATUS.NOT_FOUND);
  } catch (error32) {
...
// .wrangler/tmp/bundle-WXdwwr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-WXdwwr/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.13161661627240173.js.map
